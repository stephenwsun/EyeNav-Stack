...
//Copy the Rx Buffer contents to the Tx Buffer 
//void copyUartRxBuf2TxBuf(UARTConfig * prtInf, unsigned char * rxBuf,unsigned char * txBuf,int bufLen)

void copyUartRxBuf2TxBuf(UARTConfig * prtInf, int bufLen)
{
	//prtInf->txBuf = txBuf;
	//prtInf->txBufLen = bufLen;
	...
	for(i = 0; i < bufLen; i++)
	{
            prtInf->txBuf[i]=prtInf->rxBuf[i];
		    //txBuf[i]=rxBuf[i];
	}
}

void resetTxBuffer(UARTConfig * prtInf,int bufLen)
{
	int i = 0;
		for(i = 0; i < bufLen; i++)
		{
...
void resetRxBuffer(UARTConfig * prtInf,int bufLen)
{
	int i = 0;
		for(i = 0; i < bufLen; i++)
		{
